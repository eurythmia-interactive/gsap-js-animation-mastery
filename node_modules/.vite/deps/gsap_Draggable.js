import { o as getGlobalMatrix, t as Matrix2D } from "./matrix-CLbkHbOg.js";
//#region part-2/node_modules/gsap/Draggable.js
/*!
* Draggable 3.14.2
* https://gsap.com
*
...
var gsap, _win, _doc, _docElement, _body, _tempDiv, _placeholderDiv, _coreInitted, _checkPrefix, _toArray, _supportsPassive, _isTouchDevice, _touchEventLookup, _isMultiTouching, _isAndroid, InertiaPlugin, _defaultCursor, _supportsPointer, _context, _getStyleSaver, _dragCount = 0, _windowExists = function _windowExists() {
...
}, _RAD2DEG = 180 / Math.PI, _bigNum = 0x56bc75e2d63100000, _identityMatrix = new Matrix2D(), _getTime = Date.now || function() {
	...
	if (_isFunction(callback)) result = callback.apply(vars.callbackScope || target, vars[callbackName + "Params"] || [target.pointerEvent]);
	...
	var vars = {}, a, i, l;
	if (snap) if (factor !== 1 && snap instanceof Array) {
		vars.end = a = [];
		l = snap.length;
		...
		if (factor !== 1) if (_isObject(result)) {
			copy = {};
...
var Draggable = /* @__PURE__ */ function(_EventDispatcher) {
	...
	function Draggable(target, vars) {
		...
		var type = (vars.type || "x,y").toLowerCase(), xyMode = ~type.indexOf("x") || ~type.indexOf("y"), rotationMode = type.indexOf("rotation") !== -1, xProp = rotationMode ? "rotation" : xyMode ? "x" : "left", yProp = xyMode ? "y" : "top", allowX = !!(~type.indexOf("x") || ~type.indexOf("left") || type === "scroll"), allowY = !!(~type.indexOf("y") || ~type.indexOf("top") || type === "scroll"), minimumMovement = vars.minimumMovement || 2, self = _assertThisInitialized(_this2), triggers = _toArray(vars.trigger || vars.handle || target), killProps = {}, dragEndTime = 0, checkAutoScrollBounds = false, autoScrollMarginTop = vars.autoScrollMarginTop || 40, autoScrollMarginRight = vars.autoScrollMarginRight || 40, autoScrollMarginBottom = vars.autoScrollMarginBottom || 40, autoScrollMarginLeft = vars.autoScrollMarginLeft || 40, isClickable = vars.clickableTest || _isClickable, clickTime = 0, gsCache = target._gsap || gsap.core.getCache(target), isFixed = _isFixed(target), getPropAsNum = function getPropAsNum(property, unit) {
			...
			} else if (rotationMode) self.x = self.rotation = _round(parseFloat(gsCache.rotation));
			else if (scrollProxy) {
			...
			if (inertia && InertiaPlugin) {
				...
				if (!inertia.duration) inertia.duration = {
					...
					min: !isNaN(vars.minDuration) ? vars.minDuration : overshootTolerance === 0 || _isObject(inertia) && inertia.resistance > 1e3 ? 0 : .5,
			...
			scrollProxy && gsap.killTweensOf(target, { scrollTo: 1 }, true);
			...
			if (vars.cursor !== false || vars.activeCursor) {
				i = triggers.length;
				while (--i > -1) gsap.set(triggers[i], { cursor: vars.activeCursor || vars.cursor || (_defaultCursor === "grab" ? "grabbing" : _defaultCursor) });
			}
			_dispatchEvent(self, "press", "onPress");
			...
			if (touchEventTarget && allowNativeTouchScrolling && !touchDragAxis) {
				...
				if (dx !== dy && (dx > minimumMovement || dy > minimumMovement) || _isAndroid && allowNativeTouchScrolling === touchDragAxis) {
					...
					if (allowNativeTouchScrolling && touchDragAxis !== allowNativeTouchScrolling) _addListener(_win, "touchforcechange", _preventDefault);
			...
			var originalEvent = e, wasDragging = self.isDragging, isContextMenuRelease = self.vars.allowContextMenu && e && (e.ctrlKey || e.which > 2), placeholderDelayedCall = gsap.delayedCall(.001, removePlaceholder), touches, i, syntheticEvent, eventTarget, syntheticClick;
			if (touchEventTarget) {
			...
			if (isContextMenuRelease && originalEvent) {
				...
				if ((!_isAndroid || originalEvent.type !== "touchmove") && originalEvent.type.indexOf("cancel") === -1) {
					...
					if (!_isAndroid && !originalEvent.defaultPrevented) gsap.delayedCall(.05, syntheticClick);
				}
			} else {
		...
		_this2.update = function(applyBounds, sticky, ignoreExternalChanges) {
			if (sticky && self.isPressed) {
				if (rotationMode) self.x = self.y = _round(parseFloat(gsCache.rotation));
				else {
		...
		_this2.enable = function(type) {
			var setVars = { lazy: true }, id, i, trigger;
			...
			if (type !== "soft") {
				...
				while (--i > -1) {
					...
					if (trigger.getBBox && trigger.ownerSVGElement && allowX !== allowY) gsap.set(trigger.ownerSVGElement, { touchAction: vars.allowNativeTouchScrolling || vars.allowEventDefault ? "manipulation" : allowX ? "pan-y" : "pan-x" });
		...
		if (~type.indexOf("scroll")) {
			scrollProxy = _this2.scrollProxy = new ScrollProxy(target, _extend({ onKill: function onKill() {
